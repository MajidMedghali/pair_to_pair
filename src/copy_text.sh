...
# Assigner l'argument à la variable
output_file=$1

# Texte à répéter pour remplir le fichier
text_to_repeat="The Project Gutenberg eBook of Sonnets from the Portuguese
...
Guess \now \who holds thee!--Death, I said, But, there,
...
(1 dollars to 5,000 dollars) are particularly important to maintaining tax exempt
status with the IRS.
...
nom_dossier=$(dirname "$1")
nom_fichier=$(basename "$1")

mkdir -p $nom_dossier
touch $1
# Taille cible du fichier de sortie en octets (1 Go)
target_size=$((1024*1024*1024))  # 10 Mo en octets

# Taille initiale du fichier
current_size=0

# Remplir le fichier jusqu'à ce qu'il atteigne la taille cible
while [ $current_size -lt $target_size ]; do
    echo -n $text_to_repeat >> $output_file
